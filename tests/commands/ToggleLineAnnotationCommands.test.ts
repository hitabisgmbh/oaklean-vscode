...
import '../shared/mocks/AppConstants.mock'
...
import ToggleLineAnnotationCommands, {
	...
	CommandIdentifiers,
	ContextOptions
} from '../../src/commands/ToggleLineAnnotationCommands'
...
describe('ToggleLineAnnotationCommands', () => {
	...
	it('should return the correct identifier for disable action', () => {
		const command = new ToggleLineAnnotationCommands(
			container,
			ToggleLineAnnotationAction.disable
		)
		expect(command.getIdentifier()).toEqual(
			CommandIdentifiers.disableLineAnnotations
		)
	})
	...
	it('should return the correct identifier for enable action', () => {
		...
		expect(command.getIdentifier()).toEqual(
			CommandIdentifiers.enableLineAnnotations
		)
	})

	it('should disable line annotations', async () => {
		const fireToggleLineAnnotationsChangeSpy = jest.spyOn(
			eventHandler,
			'fireToggleLineAnnotationsChange'
		)
		const command = new ToggleLineAnnotationCommands(
			container,
			ToggleLineAnnotationAction.disable
