...
import SelectReport, {
	CommandIdentifiers
} from '../../src/commands/SelectReportFromContextMenu'
...
describe('SelectReport', () => {
	...
	test('getIdentifier returns the correct identifier', () => {
		expect(selectReport.getIdentifier()).toBe(
			CommandIdentifiers.selectReportFromContextMenu
		)
	})

	test('execute stores report path on Windows with leading slash', async () => {
		jest.spyOn(getPlatform, 'isWindows').mockReturnValue(true)
		const fireReportPathChangeEventSpy = jest.spyOn(
			eventHandler,
			'fireReportPathChange'
		)
