...
describe('TextDocumentController', () => {
	...
	describe('reportPathChanged event', () => {
		it('should handle non existing report correctly', () => {
			const loadReportSpy = jest
				.spyOn(ProjectReportHelper, 'loadReport')
				.mockReturnValue(undefined as unknown as ProjectReport)

			const reportPathChangeEvent = {
				reportPath: new UnifiedPath('/path/to/non/existend/report')
			}
			textDocumentController.reportPathChanged(reportPathChangeEvent)
			expect(textDocumentController.reportPath).toEqual(
				reportPathChangeEvent.reportPath
			)
		...
		it('should handle existing report correctly', () => {
			const loadReportSpy = jest
				.spyOn(ProjectReportHelper, 'loadReport')
				.mockReturnValue({} as unknown as ProjectReport)

			const reportPathChangeEvent = {
				reportPath: new UnifiedPath('/path/to/report')
			}
			textDocumentController.reportPathChanged(reportPathChangeEvent)
			expect(textDocumentController.reportPath).toEqual(
				reportPathChangeEvent.reportPath
	...
	it('should handle documentOpened event correctly', () => {
		const fireSourceFileInformationChangeEventSpy = jest.spyOn(
			container.eventHandler,
			'fireSourceFileInformationChange'
		)

		jest
			.spyOn(textDocumentController, 'projectReport', 'get')
			.mockReturnValue({} as ProjectReport)
		jest
			.spyOn(textDocumentController, 'reportPath', 'get')
			.mockReturnValue(new UnifiedPath('/path/to/report'))
		...
		expect(
			textDocumentController.sourceFileInformationPerDocument.keys()
		).toContain('./file.ts')

		expect(fireSourceFileInformationChangeEventSpy).toHaveBeenCalledWith(
			absoluteFilePath
		)
	})
