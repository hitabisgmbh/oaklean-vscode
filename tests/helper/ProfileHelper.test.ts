...
} from '../../src/constants/infoMessages'
...
} from '../shared/mocks/WorkspaceUtils.mock'
import {
	stub_readProfiles,
	stub_returnSettingsPath
} from '../shared/mocks/ProfileHelper.mock'
...
describe('ProfileHelper', () => {
	...
	it('should add a profile', () => {
		...
		const readProfilesSpy = jest
			.spyOn(profileHelper, 'readProfiles')
			.mockReturnValueOnce({
				profiles: []
			})
		...
		expect(writeProfilesSpy).toHaveBeenCalledWith([
			{ name: 'Profile 2', color: Color.Red, measurement: 'profilerHits' }
		])
	})
	...
	it('should not add a profile if it already exists', () => {
		...
		expect(t).toThrow('A profile with the same name already exists.')
		expect(readProfilesSpy).toHaveBeenCalled()
	...
	it('should create settings file and write profiles', () => {
		const settingsPath = new UnifiedPath(
			WORKSPACE_PATH + '.vscode/settings.json'
		)
		const returnSettingsPathSpy = jest
			.spyOn(profileHelper, 'returnSettingsPath')
			.mockReturnValueOnce(settingsPath)
		const existsSyncSpy = jest
			.spyOn(fs, 'existsSync')
			.mockReturnValueOnce(false)
		const mkdirSyncSpy = jest.spyOn(fs, 'mkdirSync')
		const writeFileSyncSpy = jest.spyOn(fs, 'writeFileSync')
		profileHelper.writeProfiles([
			{
				name: 'Profile 1',
				color: Color.Blue,
				measurement: 'aggregatedCPUTime'
		...
		expect(mkdirSyncSpy).toHaveBeenCalledWith(
			settingsPath.dirName().toPlatformString(),
			{ recursive: true }
		)
		const expectedJson = JSON.parse(
			'{"oaklean.Profiles": [{"name": "Profile 1","color": "Blue","measurement": "aggregatedCPUTime"},{"name": "Profile 2","color": "Blue","measurement": "aggregatedCPUTime"}]}'
		)
