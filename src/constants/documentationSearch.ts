export const SEARCH_DEBOUNCE_IN_MS = 150
export const SEARCH_EMPTY_TEXT = ''
export const SEARCH_RESULTS_MAX_DEFAULT = 10
export const SEARCH_RESULTS_MAX_MIN = 1
export const SEARCH_RESULTS_PAGE_SIZE_DEFAULT = 10
export const SEARCH_RESULTS_PAGE_SIZE_MIN = 1
export const SEARCH_INITIAL_OCCURRENCES_PER_DOC = 1
export const SEARCH_SNIPPET_DEFAULT_WORDS = 20
export const SEARCH_SNIPPET_WORDS_AFTER = 8
export const SEARCH_SNIPPET_WORDS_BEFORE = 8
export const SEARCH_TOKEN_SEPARATOR = ' '
export const SEARCH_MAX_RESULTS_CONFIG_KEY = 'docs.searchMaxResults'
export const SEARCH_PAGE_SIZE_CONFIG_KEY = 'docs.searchPageSize'
export const FILES_SIGNATURE_EMPTY = ''
export const FILES_SIGNATURE_ENTRY_SEPARATOR = '||'
export const FILES_SIGNATURE_PART_SEPARATOR = '::'
export const FILES_SIGNATURE_VERSION_UNSET = 'version:unset'
export const FILES_SIGNATURE_VERSION_SET = 'version:set'
export const FILES_SIGNATURE_HASH_OFFSET_BASIS_32 = 2166136261
export const FILES_SIGNATURE_HASH_PRIME_32 = 16777619
export const FILES_SIGNATURE_SAMPLE_CHARS = 256
export const FILES_SIGNATURE_SAMPLE_THRESHOLD_CHARS = 2000
export const FILES_SIGNATURE_SAMPLE_HALF_DIVISOR = 2
export const FILES_SIGNATURE_HASH_UNSIGNED_SHIFT = 0
