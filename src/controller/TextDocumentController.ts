...
export default class TextDocumentController implements Disposable {
	...
	constructor(container: Container) {
		...
		this._disposable = Disposable.from(
			this.container.eventHandler.onReportPathChange(
				this.reportPathChanged.bind(this)
			),
			this.container.eventHandler.onTextDocumentOpen(
				this.documentOpened.bind(this)
			),
			this.container.eventHandler.onTextDocumentClose(
				this.documentClosed.bind(this)
			),
			this.container.eventHandler.onTextDocumentChange(
				this.documentChanged.bind(this)
			),
			this.container.eventHandler.onTextDocumentDidSave(
				this.documentSaved.bind(this)
			)
		)
	}
	...
	getSourceFileMetaData(
		...
		return (
			this.getSourceFileInformationOfFile(relativeWorkspacePath)
				?.sourceFileMetaData || null
		)
	}
	...
	reportPathChanged(event: ReportPathChangeEvent) {
		...
		const config = WorkspaceUtils.autoResolveConfigFromReportPath(
			this.reportPath
		)
		if (config !== null) {
			this.config = config
		...
		if (this.projectReport) {
			try {
				this._sourceFileMetaDataTree = SourceFileMetaDataTree.fromProjectReport(
					this.projectReport
				)
				...
				for (const sourceFileInformation of this.sourceFileInformationPerDocument.values()) {
					sourceFileInformation.update(this.reportPath, this.projectReport)
				}
		...
		vscode.window.showInformationMessage(
			INFO_PROJECT_REPORT + this.reportPath.basename()
		)
	...
	documentSaved(document: TextDocument) {
		if (
			path.basename(document.uri.path).toLowerCase() === STATIC_CONFIG_FILENAME
		) {
			const savedConfigFilePath = new UnifiedPath(document.uri.path)
			const { config: changedConfig, error } =
				WorkspaceUtils.resolveConfigFromFile(savedConfigFilePath)
			if (changedConfig === undefined) {
				vscode.window.showWarningMessage(
					'Saved .oaklean config file has an invalid format: ' + error
				)
			}
			const currentLoadedConfigPath = this.config?.filePath
			if (
				currentLoadedConfigPath &&
				savedConfigFilePath.toString() === currentLoadedConfigPath.toString()
			) {
				if (changedConfig) {
	...
	setSourceFileInformationOfDocument(document: TextDocument) {
		...
		const existing = this.sourceFileInformationPerDocument.get(
			relativeWorkspacePath.toString()
		)
		if (existing !== undefined) {
			existing.update(this.reportPath, this.projectReport)
			this.container.eventHandler.fireSourceFileInformationChange(
		...
		if (sourceFileInformation !== undefined) {
			...
			this.container.eventHandler.fireSourceFileInformationChange(
				sourceFileInformation.absoluteFilePath
			)
		}
	}
	...
	unsetSourceFileInformationOfDocument(document: TextDocument) {
		...
		const existed = this.sourceFileInformationPerDocument.has(
			relativeWorkspacePath.toString()
		)
		this.sourceFileInformationPerDocument.delete(
			relativeWorkspacePath.toString()
		)
		if (existed) {
